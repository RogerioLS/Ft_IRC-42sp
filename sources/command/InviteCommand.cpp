...
/*   Updated: 2025/08/05 12:12:34 by pmelo-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
  ...
  bool inviteClientToChannel(Server &server, const std::string & providedChannel, const std::string & providedClientToInvite, const std::string & providedOper ) {
    ...
    while(it != server.getChannelsVector().end()) {
      if (it->getName() == providedChannel) {
        ...
        server.sendMessage(clientInstance->getClientFd(),  ":" + clientInstance->getClientNickName() + "!" + clientInstance->getClientUserName() + "@" + clientInstance->getClientipAddress() + " INVITE " + providedClientToInvite + " :" + providedChannel + "\r\n");
        return true;
      }
      ++it;
    }
